...
UTestSaveGame* UTestSaveGame::CreateSaveGameData()
{
	UTestSaveGame* SaveData = Cast<UTestSaveGame>(UGameplayStatics::CreateSaveGameObject(UTestSaveGame::StaticClass()));
...
bool UTestSaveGame::AreSaveGamesEqual(const UTestSaveGame* A, const UTestSaveGame* B)
{
	if (!A || !B)
	{
